    ...
    <div class="grid lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-3xl shadow-md border border-gray-100 p-5 sticky top-24">
                ...
                <nav class="space-y-1">
                    @foreach([['#sejarah','fa-history','Sejarah Yayasan'],['#visi-misi','fa-eye','Visi & Misi'],['#struktur','fa-sitemap','Struktur Organisasi'],['#sambutan','fa-comment-dots','Sambutan Ketua']] as $n)
                    <a href="{{ $n[0] }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-600 hover:bg-blue-50 hover:text-blue-700 font-medium text-sm transition-all group">
                        <i class="fas {{ $n[1] }} w-4 text-gray-400 group-hover:text-blue-500 transition-colors"></i>{{ $n[2] }}
                    </a>
                    @endforeach
                </nav>
            </div>
        ...
        <div class="lg:col-span-3 space-y-10">
            ...
            <section id="sejarah" class="bg-white rounded-3xl shadow-md border border-gray-100 p-8">
                <div class="flex items-center gap-3 mb-6">
                    <div class="w-12 h-12 gradient-card rounded-2xl flex items-center justify-center">
                ...
                <div class="mt-8 space-y-4">
                    ...
                    <div class="flex gap-4">
                        <div class="flex flex-col items-center">
                            ...
                            <div class="w-0.5 bg-blue-100 flex-1 mt-1"></div>
                        </div>
                        <div class="pb-4">
                            <div class="font-bold text-blue-700 text-sm">{{ $t[0] }}</div>
            ...
            <section id="visi-misi" class="bg-white rounded-3xl shadow-md border border-gray-100 p-8">
                ...
                <div class="gradient-hero rounded-2xl p-6 mb-6 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2"></div>
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 text-blue-300 font-bold text-sm mb-3 uppercase tracking-wider"><i class="fas fa-eye"></i>Visi</div>
                ...
                <div class="bg-emerald-50 rounded-2xl p-6 border border-emerald-100">
                    <div class="flex items-center gap-2 text-emerald-700 font-bold text-sm mb-4 uppercase tracking-wider"><i class="fas fa-bullseye"></i>Misi</div>
                    <ul class="space-y-3">
                        @foreach(['Menyelenggarakan pendidikan berkualitas berbasis nilai-nilai Islam','Mengembangkan potensi siswa secara holistik (akademik, spiritual, sosial)','Membangun lingkungan belajar yang kondusif dan inovatif','Menjalin kemitraan dengan orang tua, masyarakat, dan dunia usaha','Mempersiapkan lulusan yang siap menghadapi tantangan global'] as $m)
            ...
            <section id="struktur" class="bg-white rounded-3xl shadow-md border border-gray-100 p-8">
                ...
                <div class="space-y-3">
                    @foreach([
                        ...
                        ['Kepala SMA','Dr. Fatimah Zahra, M.Pd','bg-emerald-50','text-emerald-900'],
                    ] as $s)
                    <div class="flex items-center gap-4 p-4 {{ $s[2] }} rounded-2xl">
                        <div class="w-11 h-11 bg-white/30 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
                            <i class="fas fa-user {{ $s[3] }} opacity-70"></i>
                        </div>
                        <div>
                            <div class="font-extrabold {{ $s[3] }} text-sm">{{ $s[1] }}</div>
                            <div class="text-xs {{ $s[3] }} opacity-70 mt-0.5">{{ $s[0] }}</div>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <section id="sambutan" class="bg-white rounded-3xl shadow-md border border-gray-100 p-8">
                ...
                <div class="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-2xl">
                    <div class="w-16 h-16 gradient-card rounded-2xl flex items-center justify-center flex-shrink-0">

...
@php $sidebarFoto = ''; @endphp
    ...
    <div class="mt-16 reveal">
        <div class="flex items-center gap-3 mb-8">
            <div class="w-12 h-12 grad-sma rounded-xl flex items-center justify-center shadow-md">
                <i class="fas fa-medal text-white text-xl"></i>
            </div>
            <div>
                <h2 class="text-2xl font-extrabold text-gray-900">Galeri & Prestasi SMA</h2>
                <p class="text-sm text-gray-500">Dokumentasi kegiatan dan siswa berprestasi di SMA Unggulan Al-Hidayah</p>
            </div>
        </div>
        
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($galeris as $g)
            <a href="{{ route('galeri.show', $g->id) }}" class="group block bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-sky-200 transition-all duration-300">
                ...
                <div class="p-4">
                    <h3 class="font-bold text-sm text-gray-900 group-hover:text-sky-600 transition-colors line-clamp-2 leading-snug">{{ $g->judul }}</h3>
                </div>
            </a>
            @endforeach
        </div>
